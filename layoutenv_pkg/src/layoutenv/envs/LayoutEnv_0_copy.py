...
class LayoutEnv(Env):
    ...
    def step(self, action):
        ...
        done = self._terminated(reward) | self._truncated(max_time_steps=30)
        return observation, reward, done, info

    def render(self):
        c = Client()
        self.renderer = RenderLayoutModule(source=self.config["temp_file"], serverport=self.config['serverport'], servermode=self.config['servermode'])
        copy = Copy()
    ...
    def reset(self):
        ...
        default = [("Frame 80 (40.000 m)", 13), ("Frame 160 (80.000 m)", 16), ("Deck 4.000 m", 18), ("Deck 1.000 m", 17), ("Longitudinal bulkhead 0.000 m", 19), ("Longitudinal bulkhead 2.350 m", 20), ("Longitudinal bulkhead -2.350 m", 21), ("Longitudinal bulkhead 4.000 m", 22), ("Longitudinal bulkhead -4.000 m", 23) ]
